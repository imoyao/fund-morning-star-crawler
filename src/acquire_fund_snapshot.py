    ...
    while page_index <= page_total:
        ...
        for i in range(len(class_list)):
            for tr in bs.find_all('tr', {'class': class_list[i]}):
                ...
                # 三年评级
                rating_3y = get_star_count(tds_text[3].find_all('img')[0]['src'])
                fund_rating_3.append(rating_3y)
                # 5年评级
                rating_5y = get_star_count(tds_text[4].find_all('img')[0]['src'])
                fund_rating_5.append(rating_5y)
                # 今年以来回报(%)
        ...
        fund_df = pd.DataFrame(
            {'id': id_list, 'fund_code': code_list, 'morning_star_code': morning_star_code_list, 'fund_name': name_list,
             'fund_cat': fund_cat,
        ...
        fund_df_no_nan = fund_df.fillna(np.nan).replace([np.nan], [None])
        fund_list = fund_df_no_nan.values.tolist()
        try:
            cursor.executemany(sql_insert, fund_list)
            connect_instance.commit()
        except pymysql.err.ProgrammingError as e:
            print(f'Insert fund_list: {fund_list} with Error: {e}')
            break
        # 输出为csv文件
